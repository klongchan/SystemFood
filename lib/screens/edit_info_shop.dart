...
class _EditInfoShopState extends State<EditInfoShop> {
  ...
  Future<Null> confirmDialog() async {
    showDialog(
      ...
      builder: (context) => SimpleDialog(
        title: Text('คุณต้องการที่จะ ปรับปรุงรายละเอียดร้าน'),
        children: <Widget>[
          Row(
  ...
  Future<Null> editThread() async {
    ...
    String urlUpload = '${MyConstant().domain}/RiwFood/saveshop.php';
    await Dio().post(urlUpload, data: formData).then((value) async {
      urlPicture = '/RiwFood/shop/$nameFile';

      String id = userModel.id;
      // print('id = $id');

      String url =
          '${MyConstant().domain}/RiwFood/editUserWhereId.php?isAdd=true&id=$id&NameShop=$nameShop&Address=$address&Phone=$phone&UrlPicture=$urlPicture&Lat=$lat&Lng=$lng';

      Response response = await Dio().get(url);
      if (response.toString() == 'true') {
        Navigator.pop(context);
      } else {
        normalDialog(context, 'ยัง Update ไม่ได้ กรุณาลองอีกครั้ง');
      }
    });
  }
