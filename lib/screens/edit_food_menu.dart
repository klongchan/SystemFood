...
class _EditFoodMenuState extends State<EditFoodMenu> {
    ...
    return FloatingActionButton(
      onPressed: () {
        if (name.isEmpty || price.isEmpty || detail.isEmpty) {
          normalDialog(context, 'กรุณากรอกให้ครบ ทุกช่องค่ะ');
        } else {
          confirmEdit();
        }
      },
  ...
  Future<Null> confirmEdit() async {
    showDialog(
      ...
      builder: (context) => SimpleDialog(
        title: Text('คุณต้องการจะ เปลี่ยนแปลง เมนูอาหาร แน่นอน นะ ?'),
        children: <Widget>[
          Row(
            children: <Widget>[
              FlatButton.icon(
  ...
  Future<Null> editValueOnMySQL() async {
    ...
    String url =
        '${MyConstant().domain}/RiwFood/editFoodWhereId.php?isAdd=true&id=$id&NameFood=$name&PathImage=$pathImage&Price=$price&Detail=$detail';
  ...
  Widget groupImage() => Row(
        ...
        children: <Widget>[
          IconButton(
            ...
            onPressed: () => chosseImage(ImageSource.camera),
          ),
          Container(
          ...
          IconButton(
            ...
            onPressed: () => chosseImage(ImageSource.gallery),
          ),
        ],
      );

  Future<Null> chosseImage(ImageSource source) async {
    try {
