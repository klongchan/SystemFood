...
import 'package:systemfood/screens/add_food_menu.dart';
import 'package:systemfood/screens/edit_food_menu.dart';
...
class _ListFoodMenuShopState extends State<ListFoodMenuShop> {
  bool loadStatus = true; //ที่โหลดไม่เสร็จ
  ...
  Future<Null> deleteFood(FoodModel foodModel) async {
    showDialog(
      ...
      builder: (context) => SimpleDialog(
        title: MyStyle().showTitleH2('ต้องการลบ เมนู ${foodModel.nameFood} ?'),
        children: <Widget>[
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceAround,
            children: <Widget>[
              FlatButton(
                onPressed: () async {
                  ...
                  String url =
                      '${MyConstant().domain}/RiwFood/deleteFoodWhereId.php?isAdd=true&id=${foodModel.id}';
